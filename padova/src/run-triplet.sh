# ===========> EUC DISTANCE
# python train_triplet.py --model full --bs 256 --lr 0.01     --margin 1.5
# python train_triplet.py --model full --bs 256 --lr 0.001    --margin 1.5
# python train_triplet.py --model full --bs 128 --lr 0.01     --margin 1.5
# python train_triplet.py --model full --bs 128 --lr 0.001    --margin 1.5

# python train_triplet.py --model full --bs 256 --lr 0.01     --margin 5.0
# python train_triplet.py --model full --bs 256 --lr 0.001    --margin 5.0
# python train_triplet.py --model full --bs 128 --lr 0.01     --margin 5.0
# python train_triplet.py --model full --bs 128 --lr 0.001    --margin 5.0

# python train_triplet.py --model full --bs 256 --lr 0.01     --margin 10.0
# python train_triplet.py --model full --bs 256 --lr 0.001    --margin 10.0
# python train_triplet.py --model full --bs 128 --lr 0.01     --margin 10.0
# python train_triplet.py --model full --bs 128 --lr 0.001    --margin 10.0


# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 1.5   --dreg const    --dmax 0.2   --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 1.5   --dreg const    --dmax 0.4   --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 1.5   --dreg const    --dmax 0.6   --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 1.5   --dreg square    --dmax 0.2  --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 1.5   --dreg square    --dmax 0.4  --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 1.5   --dreg square    --dmax 0.6  --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 1.5   --dreg log       --dmax 0.2  --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 1.5   --dreg log       --dmax 0.4  --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 1.5   --dreg log       --dmax 0.6  --dmaxep 20

# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 3.0   --dreg const    --dmax 0.2   --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 3.0   --dreg const    --dmax 0.4   --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 3.0   --dreg const    --dmax 0.6   --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 3.0   --dreg square    --dmax 0.2  --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 3.0   --dreg square    --dmax 0.4  --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 3.0   --dreg square    --dmax 0.6  --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 3.0   --dreg log    --dmax 0.2     --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 3.0   --dreg log    --dmax 0.4     --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 3.0   --dreg log    --dmax 0.6     --dmaxep 20

# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 5.0   --dreg const    --dmax 0.2   --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 5.0   --dreg const    --dmax 0.4   --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 5.0   --dreg const    --dmax 0.6   --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 5.0   --dreg square    --dmax 0.2  --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 5.0   --dreg square    --dmax 0.4  --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 5.0   --dreg square    --dmax 0.6  --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 5.0   --dreg log    --dmax 0.2     --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 5.0   --dreg log    --dmax 0.4     --dmaxep 20
# python train_triplet.py --model ter --bs 256 --lr 0.01      --margin 5.0   --dreg log    --dmax 0.6     --dmaxep 20


# python train_triplet.py --model bin --bs 256 --lr 0.01      --margin 1.5
# python train_triplet.py --model bin --bs 256 --lr 0.001     --margin 1.5
# python train_triplet.py --model bin --bs 128 --lr 0.01      --margin 1.5
# python train_triplet.py --model bin --bs 128 --lr 0.001     --margin 1.5

# python train_triplet.py --model bin --bs 256 --lr 0.01      --margin 5.0
# python train_triplet.py --model bin --bs 256 --lr 0.001     --margin 5.0
# python train_triplet.py --model bin --bs 128 --lr 0.01      --margin 5.0
# python train_triplet.py --model bin --bs 128 --lr 0.001     --margin 5.0

# python train_triplet.py --model bin --bs 256 --lr 0.01      --margin 10.0
# python train_triplet.py --model bin --bs 256 --lr 0.001     --margin 10.0
# python train_triplet.py --model bin --bs 128 --lr 0.01      --margin 10.0
# python train_triplet.py --model bin --bs 128 --lr 0.001     --margin 10.0


# ===========> COS DISTANCE

# python train_triplet.py --model full --dist cos --bs 256 --lr 0.001    --margin 0.1
# python train_triplet.py --model full --dist cos --bs 256 --lr 0.001    --margin 0.2
# python train_triplet.py --model full --dist cos --bs 256 --lr 0.001    --margin 0.3
# python train_triplet.py --model full --dist cos --bs 256 --lr 0.001    --margin 0.4
# python train_triplet.py --model full --dist cos --bs 256 --lr 0.001    --margin 0.6
# python train_triplet.py --model full --dist cos --bs 256 --lr 0.001    --margin 0.8
# python train_triplet.py --model full --dist cos --bs 256 --lr 0.001    --margin 1.0

# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.1
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.2
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.3
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.4
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.6
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.8
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 1.0

# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.01
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.04
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.08

# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.01
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.04
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.08

# python train_triplet.py --model ter --dist cos --bs 256 --lr 0.01      --margin 0.1 --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist cos --bs 256 --lr 0.01      --margin 0.2 --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist cos --bs 256 --lr 0.01      --margin 0.3 --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist cos --bs 256 --lr 0.01      --margin 0.4 --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist cos --bs 256 --lr 0.01      --margin 0.6 --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist cos --bs 256 --lr 0.01      --margin 0.8 --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist cos --bs 256 --lr 0.01      --margin 1.0 --dreg log --dmax 0.2  --dmaxep 50


## ===========> RERUN (SMALL MARGIN)
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.001
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.004
# python train_triplet.py --model bin --dist cos --bs 128 --lr 0.01      --margin 0.008

# python train_triplet.py --model ter --dist cos --bs 128 --lr 0.01      --margin 0.01
# python train_triplet.py --model ter --dist cos --bs 128 --lr 0.01      --margin 0.04
# python train_triplet.py --model ter --dist cos --bs 128 --lr 0.01      --margin 0.08
# python train_triplet.py --model ter --dist cos --bs 128 --lr 0.01      --margin 0.001
# python train_triplet.py --model ter --dist cos --bs 128 --lr 0.01      --margin 0.004
# python train_triplet.py --model ter --dist cos --bs 128 --lr 0.01      --margin 0.008


## ===========> BINARY HAMMING TRAINING

# python train_triplet.py --model bin --bs 128 --lr 0.001 --dist hamm --margin 0.001
# python train_triplet.py --model bin --bs 128 --lr 0.001 --dist hamm --margin 0.004
# python train_triplet.py --model bin --bs 128 --lr 0.001 --dist hamm --margin 0.008
# python train_triplet.py --model bin --bs 128 --lr 0.001 --dist hamm --margin 0.01
# python train_triplet.py --model bin --bs 128 --lr 0.001 --dist hamm --margin 0.04
# python train_triplet.py --model bin --bs 128 --lr 0.001 --dist hamm --margin 0.08
# python train_triplet.py --model bin --bs 128 --lr 0.001 --dist hamm --margin 0.1
# python train_triplet.py --model bin --bs 128 --lr 0.001 --dist hamm --margin 0.2
# python train_triplet.py --model bin --bs 128 --lr 0.001 --dist hamm --margin 0.3
# python train_triplet.py --model bin --bs 128 --lr 0.001 --dist hamm --margin 0.4


# python train_triplet.py --model ter --dist hamm --bs 128 --lr 0.001      --margin 0.001  --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist hamm --bs 128 --lr 0.001      --margin 0.004  --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist hamm --bs 128 --lr 0.001      --margin 0.008  --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist hamm --bs 128 --lr 0.001      --margin 0.01   --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist hamm --bs 128 --lr 0.001      --margin 0.04   --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist hamm --bs 128 --lr 0.001      --margin 0.08   --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist hamm --bs 128 --lr 0.001      --margin 0.1    --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist hamm --bs 128 --lr 0.001      --margin 0.2    --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist hamm --bs 128 --lr 0.001      --margin 0.3    --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist hamm --bs 128 --lr 0.001      --margin 0.4    --dreg log --dmax 0.2  --dmaxep 50


## ===========> TER BEST LOG COS
# python train_triplet.py --model ter --dist cos --bs 128 --lr 0.01      --margin 0.01  --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist cos --bs 128 --lr 0.01      --margin 0.04  --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist cos --bs 128 --lr 0.01      --margin 0.08  --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist cos --bs 128 --lr 0.01      --margin 0.001 --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist cos --bs 128 --lr 0.01      --margin 0.004 --dreg log --dmax 0.2  --dmaxep 50
# python train_triplet.py --model ter --dist cos --bs 128 --lr 0.01      --margin 0.008 --dreg log --dmax 0.2  --dmaxep 50

## ===========> OPEN SET

python train_triplet.py --model full --bs 256 --lr 0.001     --dist euc  --margin 1.5      --open 0
python train_triplet.py --model full --bs 256 --lr 0.001     --dist euc  --margin 5.0     --open 0
python train_triplet.py --model full --bs 256 --lr 0.001     --dist euc  --margin 10.0     --open 0
python train_triplet.py --model bin  --bs 128 --lr 0.01      --dist euc  --margin 1.5      --open 0   
python train_triplet.py --model bin  --bs 128 --lr 0.01      --dist euc  --margin 5.0     --open 0   
python train_triplet.py --model bin  --bs 128 --lr 0.01      --dist euc  --margin 10.0     --open 0   
python train_triplet.py --model ter  --bs 256 --lr 0.01      --dist euc  --margin 1.5      --open 0  --dmaxep 50 --dmax 0.2 --dreg const
python train_triplet.py --model ter  --bs 256 --lr 0.01      --dist euc  --margin 5.0     --open 0  --dmaxep 50 --dmax 0.2 --dreg const
python train_triplet.py --model ter  --bs 256 --lr 0.01      --dist euc  --margin 10.0     --open 0  --dmaxep 50 --dmax 0.2 --dreg const

python train_triplet.py --model full --bs 256 --lr 0.001     --dist cos  --margin 0.008      --open 0
python train_triplet.py --model full --bs 256 --lr 0.001     --dist cos  --margin 0.01      --open 0
python train_triplet.py --model full --bs 256 --lr 0.001     --dist cos  --margin 0.04     --open 0
python train_triplet.py --model full --bs 256 --lr 0.001     --dist cos  --margin 0.08      --open 0
python train_triplet.py --model full --bs 256 --lr 0.001     --dist cos  --margin 0.1      --open 0
python train_triplet.py --model full --bs 256 --lr 0.001     --dist cos  --margin 0.2      --open 0
python train_triplet.py --model bin  --bs 128 --lr 0.01     --dist cos  --margin 0.008      --open 0
python train_triplet.py --model bin  --bs 128 --lr 0.01     --dist cos  --margin 0.01      --open 0
python train_triplet.py --model bin  --bs 128 --lr 0.01     --dist cos  --margin 0.04     --open 0
python train_triplet.py --model bin  --bs 128 --lr 0.01     --dist cos  --margin 0.08      --open 0
python train_triplet.py --model bin  --bs 128 --lr 0.01     --dist cos  --margin 0.1      --open 0
python train_triplet.py --model bin  --bs 128 --lr 0.01     --dist cos  --margin 0.2      --open 0
python train_triplet.py --model ter  --bs 256 --lr 0.01     --dist cos  --margin 0.008      --open 0    --dmaxep 50 --dmax 0.2 --dreg const
python train_triplet.py --model ter  --bs 256 --lr 0.01     --dist cos  --margin 0.01      --open 0     --dmaxep 50 --dmax 0.2 --dreg const
python train_triplet.py --model ter  --bs 256 --lr 0.01     --dist cos  --margin 0.04     --open 0      --dmaxep 50 --dmax 0.2 --dreg const
python train_triplet.py --model ter  --bs 256 --lr 0.01     --dist cos  --margin 0.08      --open 0     --dmaxep 50 --dmax 0.2 --dreg const
python train_triplet.py --model ter  --bs 256 --lr 0.01     --dist cos  --margin 0.1      --open 0      --dmaxep 50 --dmax 0.2 --dreg const
python train_triplet.py --model ter  --bs 256 --lr 0.01     --dist cos  --margin 0.2      --open 0      --dmaxep 50 --dmax 0.2 --dreg const 

python train_triplet.py --model bin  --bs 128 --lr 0.01     --dist hamm  --margin 0.008      --open 0
python train_triplet.py --model bin  --bs 128 --lr 0.01     --dist hamm  --margin 0.01      --open 0
python train_triplet.py --model bin  --bs 128 --lr 0.01     --dist hamm  --margin 0.04     --open 0
python train_triplet.py --model bin  --bs 128 --lr 0.01     --dist hamm  --margin 0.08      --open 0
python train_triplet.py --model bin  --bs 128 --lr 0.01     --dist hamm  --margin 0.1      --open 0
python train_triplet.py --model bin  --bs 128 --lr 0.01     --dist hamm  --margin 0.2      --open 0
python train_triplet.py --model ter  --bs 256 --lr 0.01     --dist hamm  --margin 0.008      --open 0    --dmaxep 50 --dmax 0.2 --dreg const
python train_triplet.py --model ter  --bs 256 --lr 0.01     --dist hamm  --margin 0.01      --open 0     --dmaxep 50 --dmax 0.2 --dreg const
python train_triplet.py --model ter  --bs 256 --lr 0.01     --dist hamm  --margin 0.04     --open 0      --dmaxep 50 --dmax 0.2 --dreg const
python train_triplet.py --model ter  --bs 256 --lr 0.01     --dist hamm  --margin 0.08      --open 0     --dmaxep 50 --dmax 0.2 --dreg const
python train_triplet.py --model ter  --bs 256 --lr 0.01     --dist hamm  --margin 0.1      --open 0      --dmaxep 50 --dmax 0.2 --dreg const
python train_triplet.py --model ter  --bs 256 --lr 0.01     --dist hamm  --margin 0.2      --open 0      --dmaxep 50 --dmax 0.2 --dreg const 
